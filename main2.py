...
def train(model_path,history_path):
...
def results(model_path,history_path):
    ...
    m1 = tf.keras.models.load_model(model_path)
    # ---- RESULTS ----
    # visualize accuracy
    ...
    for i in np.arange(0, L * W):
        ...
        axes[i].set_title(
            "Prediction Class =" + str(round(y_predicted[b[i]][1], 5)) + ", Actual Label = " + str(y_actual[b[i]]))
        axes[i].axis('off')
...
train('meningioma_tumor_model')

results('meningioma_tumor_model')
